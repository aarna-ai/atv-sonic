...
describe('Factory Contract', (accounts) => {
    ...
    before(async () => {
        ...
        await expect(AFiFacotry.deploy(constants.ZERO_ADDRESS)).to.be.reverted;
        ...
        console.log("deployedBase", deployedAFiBase);
        console.log("result++++++++++++++++++++++++", txObject.events[5].address);
        ...
        const totalVault = await aFiFactoryInstance.afiProductsCount();
        ...
        */
        ...
        console.log("usdtBalance", usdtBalance)
        await usdtConInstance.connect(signer).transfer(investor1.address, "2426360646");
        await usdtConInstance.connect(signer).transfer(investor2.address, "2426360646");
    ...
    context('Reverts', () => {
        it('should revert if Manager address is zero', async () => {
            await expect(
                aFiFactoryInstance.createAToken("AFiBase", "ATOK", bytesPayload2, [investor1.address], false, aFiStorageInstance.address,
                    aFiPassiveRebalanceInstance.address, constants.ZERO_ADDRESS, [constants.ZERO_ADDRESS], "0x0000000000000000000000000000000000000000")
            ).to.be.reverted;
        });
        ...
        it('should revert if team wallets address is zero', async () => {
            await expect(
                aFiFactoryInstance.createAToken("AFiBase", "ATOK", bytesPayload2, [constants.ZERO_ADDRESS], false, aFiStorageInstance.address,
                    aFiPassiveRebalanceInstance.address, aFiManagerInstance.address, [investor1.address], "0x0000000000000000000000000000000000000000")
            ).to.be.reverted;
        });
        ...
        it('should revert if array length mismatched', async () => {
            ...
            await expect(
                aFiFactoryInstance.createAToken("AFiBase", "ATOK", MismatchedPayloadbytes2, [investor1.address], false, aFiStorageInstance.address,
                    aFiPassiveRebalanceInstance.address, aFiManagerInstance.address, [investor1.address], "0x0000000000000000000000000000000000000000", { gas: 2000000 })
            ).to.be.reverted;
        });
        ...
        it('should revert if underlying address is zero', async () => {
            ...
            await expect(
                aFiFactoryInstance.createAToken("AFiBase", "ATOK", zeroPayloadbytes2, [investor1.address], false, aFiStorageInstance.address,
                    aFiPassiveRebalanceInstance.address, aFiManagerInstance.address, [investor1.address], "0x0000000000000000000000000000000000000000", { gas: 2000000 }), "zero addr"
            ).to.be.reverted;
        });
        ...
        it('should revert proportion sum is equal to zero', async () => {
            ...
            try {
                await aFiFactoryInstance.createAToken("AFiBase", "ATOK", payloadbytes, [investor1.address, investor2.address], false, aFiStorageInstance.address,
                    aFiPassiveRebalanceInstance.address, aFiAFiOracleInstance.address, ["0xdAC17F958D2ee523a2206206994597C13D831ec7"], "0x0000000000000000000000000000000000000000");
        ...
        it('should revert if array lengths are not equal', async () => {
            ...
            await expect(aFiFactoryInstance.createAToken("AFiBase", "ATOK", invalidPayload, [investor1.address], false, aFiStorageInstance.address,
                aFiPassiveRebalanceInstance.address, aFiManagerInstance.address, [investor1.address], "0x0000000000000000000000000000000000000000")).to.be.reverted;
    ...
    context('Basic check for states and deployment of product', async () => {
        ...
        beforeEach(async () => {
            result = await aFiFactoryInstance.createAToken("AFiBase", "ATOK", bytesPayload2, [investor1.address, investor2.address], true, aFiStorageInstance.address,
                aFiPassiveRebalanceInstance.address, aFiManagerInstance.address, ["0xdAC17F958D2ee523a2206206994597C13D831ec7", "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48", "0x6B175474E89094C44Da98b954EedeAC495271d0F"], "0x0000000000000000000000000000000000000000");
            //deployedAFiBase = await aFiFactoryInstance.afiProductsCount()
            let txObject = await result.wait()
        ...
        it('should deploy successfully', async () => {
            result = await aFiFactoryInstance.createAToken("AFiBase", "ATOK", bytesPayload2, [investor1.address, investor2.address], false, aFiStorageInstance.address,
                aFiPassiveRebalanceInstance.address, aFiAFiOracleInstance.address, [], "0x0000000000000000000000000000000000000000");
            let txObject = await result.wait()
        ...
        it('initialize afiVault with another wallet', async function () {
            await expect(
                aFiFactoryInstance.connect(investor1).afiContractInitUpdate(
                    aTokenConInstance.address, 1
                )
